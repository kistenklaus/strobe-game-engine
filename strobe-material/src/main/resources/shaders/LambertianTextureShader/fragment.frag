...
void main(){
    ...
    vec3 materialDiffuse = material.diffuseColor * texture(diffuseTexture, uv).rgb;

    vec3 combined = vec3(0);

    for (int i=0;i<directionalLightCount;i++){
        combined += calcDirLight(directionalLightDir[i], directionalLightDiffuse[i], norm, materialDiffuse);
    }
