...
public:
  static constexpr std::size_t
      DEFAULT_NORMAL_CHUNK_SIZE = 4096;
  ...
  InplaceMonotonicResource(
      const InplaceMonotonicResource&
  ) = delete;

  InplaceMonotonicResource& operator=(
      const InplaceMonotonicResource&
  ) = delete;
  ...
  void* allocate(
    ...
    if (size > m_normal_chunk_size) {
      return allocate_dedicated(
          size,
          alignment
      );
    }

    allocate_regular_chunk(
        m_normal_chunk_size,
        alignment
    );
...
private:
  ...
  Chunk* allocate_chunk(
    ...
    assert(
        is_power_of_two(allocation_alignment)
    );
...
private:
  ...
  std::array<std::byte, InlineBytes>
      m_inline_storage{};
