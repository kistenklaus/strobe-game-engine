...
template <typename T, Allocator A> struct ControlBlock {
  T m_value;
  ...
  explicit ControlBlock(A alloc, Args &&...args)
      : m_value(std::forward<Args>(args)...),
        m_referenceCounter(std::size_t(0)), m_allocator(std::move(alloc)) {}
};
} // namespace block::details

template <typename T, Allocator A> class Block;

template <typename T, Allocator A> class BlockRef {
...
// unique ptr, with the additional capabiltiy to query if
// a weak ptr on it exist (i.e. isReferenced)
template <typename T, Allocator A> class Block {
private:
...
public:
  ...
  template <typename... Args> static Block make(A alloc, Args &&...args) {
