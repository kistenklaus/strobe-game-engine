...
VertexShader create_vertex(Context context, const VertexShaderInfo &info,
  ...
  const vulkan::ShaderObject so = create_shader_object(
      context, ShaderStage::vertex, info.nextStage, info.spirv);
  return detail::make_object<VertexShader>(make_void_handle<ShaderObjectImpl>(
...
FragmentShader create_fragment(Context context, const FragmentShaderInfo &info,
  ...
  const vulkan::ShaderObject so = create_shader_object(
      context, ShaderStage::fragment, ShaderStage::none, info.spirv);
  return detail::make_object<FragmentShader>(make_void_handle<ShaderObjectImpl>(
      &alloc->shaderAlloc, std::move(context), so));
}
...
ComputeShader create_compute(Context context, const ComputeShaderInfo &info,
  ...
  const vulkan::ShaderObject so = create_shader_object(
      context, ShaderStage::compute, ShaderStage::none, info.spirv);
  return detail::make_object<ComputeShader>(make_void_handle<ShaderObjectImpl>(
      &alloc->shaderAlloc, std::move(context), so));
}
