...
struct DeviceImpl {
  DeviceImpl(const DeviceCreateInfo &createInfo)
      : context(vulkan::ContextCreateInfo{
            ...
            .surface = createInfo.swapchain ? vulkan::feature::required
                                            : vulkan::feature::optional,
            .swapchain = createInfo.swapchain ? vulkan::feature::required
                                              : vulkan::feature::optional,
    ...
    for (uint32_t i = 0; i < NATIVE_QUEUE_COUNT; ++i) {
      nativeQueues[i].queue = context.ctx()->queue(i);
      nativeQueues[i].timeline =
          vulkan::create_timeline_semaphore(context.ctx(), {.initalValue = 0});
    }
  ...
  ~DeviceImpl() {
    ...
    for (NativeQueue &queue : nativeQueues) {
      if (queue.timeline) {
        vulkan::destroy_timeline_semaphore(context.ctx(), queue.timeline);
  ...
  void gc_thread_main(std::stop_token st) {
    tracy::SetThreadName("strobe-rhi-gc");
    ...
    vulkan::TimelineSemaphore wakeSemaphore =
        vulkan::create_timeline_semaphore(context.ctx(), {.initalValue = 0});
    vulkan::set_debug_name(context.ctx(), wakeSemaphore, "gc-thread-wake");
...

    {
      std::stop_callback stopCallback{
          st, [device = context.ctx()->device(), wakeSemaphore]() noexcept {
      ...
      while (!st.stop_requested()) {
        ...
        VkResult result;
        {
          ...
          result = vkWaitSemaphores(context.ctx()->device(), &waitInfo,
        ...
        for (uint32_t i = 0; i < NATIVE_QUEUE_COUNT; ++i) {
...

          {
            ...
            result = vkGetSemaphoreCounterValue(context.ctx()->device(),
    ...
    vulkan::destroy_timeline_semaphore(context.ctx(), wakeSemaphore);
  }
