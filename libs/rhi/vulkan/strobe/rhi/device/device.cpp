...
#include "strobe/rhi/command/command_pool_impl.hpp"
...
#include "strobe/rhi/queue/timeline_semaphore_impl.hpp"
...
Swapchain Device::create_swapchain(GLFWwindow *window,
  ...
  auto *impl = void_handle_ptr<DeviceImpl>(m_handle);
  vulkan::Surface surface = vulkan::create_surface(impl->context.ctx(), window);
  Surface surface_handle{make_void_handle<SurfaceImpl>(impl->context, surface)};

  const vulkan::SurfaceCapabilities capabilities =
      vulkan::query_surface_capabilities(impl->context.ctx(), surface);
  ...
  auto formats = vulkan::query_surface_formats(impl->context.ctx(), surface);
  if (formats.empty()) {
    vulkan::destroy_surface(impl->context.ctx(), surface);
  ...
  auto present_modes =
      vulkan::query_present_modes(impl->context.ctx(), surface);
...
BinarySemaphore Device::create_binary_semaphore() {
  ...
  vulkan::BinarySemaphore sem =
      vulkan::create_binary_semaphore(impl->context.ctx());
...
TimelineSemaphore Device::create_timeline_semaphore(uint64_t initalValue) {
  ...
  vulkan::TimelineSemaphore sem = vulkan::create_timeline_semaphore(
      impl->context.ctx(), {.initalValue = initalValue});
...
VertexShader Device::create_vertex_shader(const VertexShaderInfo &info) {
  ...
  vulkan::ShaderObject so = vulkan::create_shader_object(
      impl->context.ctx(), vulkan::ShaderObjectCreateInfo{
