 ...
 * \snippet{cpp} pipeline_stage.hpp PipelineStage
 *
...
[[nodiscard]] constexpr PipelineStage operator~(PipelineStage value) noexcept {
  return static_cast<PipelineStage>(
      ~static_cast<std::underlying_type_t<PipelineStage>>(value));
}

constexpr PipelineStage &operator|=(PipelineStage &lhs,
                                    PipelineStage rhs) noexcept {
  lhs = lhs | rhs;
  return lhs;
}
...
operator==(std::underlying_type_t<PipelineStage> lhs,
           PipelineStage rhs) noexcept {
  return lhs == static_cast<std::underlying_type_t<PipelineStage>>(rhs);
}
