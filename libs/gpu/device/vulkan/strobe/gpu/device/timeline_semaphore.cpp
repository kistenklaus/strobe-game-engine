...
uint64_t TimelineSemaphore::count() const {
  ...
  return vulkan::get_timeline_semaphore_value(impl->context.get(),
                                              impl->semaphore);
}
...
void TimelineSemaphore::signal(uint64_t value) {
  ...
  auto *impl = void_handle_ptr<TimelineSemaphoreImpl>(m_handle);
  vulkan::signal_timeline_semaphore(impl->context.get(), impl->semaphore,
                                    value);
}
...
bool TimelineSemaphore::wait(uint64_t value, uint64_t timeout) {
  ...
  auto *impl = void_handle_ptr<TimelineSemaphoreImpl>(m_handle);
  return vulkan::wait_for_timeline_semaphore(impl->context.get(),
