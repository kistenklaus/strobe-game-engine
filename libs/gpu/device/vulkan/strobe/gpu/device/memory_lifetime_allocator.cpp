...
VkDeviceSize MemoryLifetimeAllocator::greedy_pack_intervals(
    std::span<AllocationInfo *> allocations,
  ...
  static constexpr uint32_t EVENT_BEGIN_BIT =
      uint32_t{1} << 31;

  static constexpr uint32_t EVENT_INDEX_MASK =
      ~EVENT_BEGIN_BIT;
  ...
  const AllocationInfo *referenceInfo =
      allocations.front();

  assert(referenceInfo != nullptr);

  const VkDeviceSize commonAlignment =
      referenceInfo
          ->reservation
          .alignment;

  const VkDeviceSize referenceSize =
      referenceInfo->size;

  const uint32_t referencePosition =
      referenceInfo
          ->lifetime.begin;

  const MemoryGranularityClass
      commonGranularity =
          referenceInfo
              ->granularity;
  ...
  static constexpr size_t
      RADIX_SORT_EVENT_THRESHOLD =
          256;

  const bool useRadixSort =
      eventCount >=
      RADIX_SORT_EVENT_THRESHOLD;

  const size_t holeCapacity =
      intervalCount + 1;

  const bool ownsPlacementOffsets =
      offsets.empty();

  const size_t radixEventSize =
      eventCount *
      sizeof(Event);
  ...
  const size_t radixSizeKeySize =
      intervalCount *
      sizeof(uint64_t);

  const size_t radixAlignmentKeyOffset =
      radixSizeKeyOffset +
      radixSizeKeySize;

  const size_t radixAlignmentKeySize =
      intervalCount *
      sizeof(uint8_t);

  const size_t radixStorageSize =
      radixAlignmentKeyOffset +
      radixAlignmentKeySize;
  ...
  const size_t activeNodeOffset =
      reserve_scratch(
          intervalCount *
              sizeof(ActiveRangeNode),
          alignof(ActiveRangeNode));

  size_t placementOffsetOffset = 0;

  if (ownsPlacementOffsets) {
    placementOffsetOffset =
        reserve_scratch(
            intervalCount *
                sizeof(VkDeviceSize),
  ...
  auto *activeNodes =
      reinterpret_cast<ActiveRangeNode *>(
          scratchBytes +
          activeNodeOffset);
    ...
    if (!useRadixSort) {
      std::sort(
          ...
          [&](const Event &a,
            ...
            if (aBegins) {
              const AllocationInfo *aInfo =
                  allocations[aIndex];

              const AllocationInfo *bInfo =
                  allocations[bIndex];
      ...
      for (uint32_t shift = 0;
        ...
        const uint64_t digitMask =
            static_cast<uint64_t>(
                bucketCount - 1);

        radix_pass(
            [&](const Event &event) {
              if ((event.indexAndFlags &
                   EVENT_BEGIN_BIT) == 0) {
                return uint32_t{0};
              }
...

  {
    ...
    for (size_t eventIndex = 0;
      ...
      if (begins) {
        ...
        size_t selectedHoleIndex =
            holeCount;

        VkDeviceSize placementOffset = 0;

        auto find_hole =
            [&](bool applyGranularity) {
              for (size_t holeIndex = 0;
                ...
                VkDeviceSize usableBegin =
                    hole.begin;

                VkDeviceSize usableEnd =
                    hole.end;
        ...
        const VkDeviceSize allocationEnd =
            placementOffset +
            info.size;
        ...
        ActiveRangeNode *leftActive =
            selectedHole.leftActive;

        ActiveRangeNode *rightActive =
            selectedHole.rightActive;
      ...
      const VkDeviceSize allocationBegin =
          placementOffsets[
              intervalIndex];
      ...
      const bool hasRightHole =
          insertionIndex <
              holeCount &&
          holes[insertionIndex]
                  .begin ==
              allocationEnd;
