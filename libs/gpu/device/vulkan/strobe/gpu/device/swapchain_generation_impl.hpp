...
struct SwapchainGenerationImpl {
  ...
  ~SwapchainGenerationImpl() noexcept {
    ...
    for (vulkan::CommandPool cmdPool : cmdPools) {
      if (cmdPool) {
        vulkan::destroy_command_pool(surface_impl->context.get(), cmdPool);
      }
    }

    vulkan::destroy_swapchain(surface_impl->context.get(), swapchain);
  }
  ...
  void ensure_transition_cache(uint32_t queueFamilyIndex) {
...

    {
      ...
      VkResult result = vkCreateCommandPool(
          surfaceImpl->context.get()->device(), &createInfo,
          surfaceImpl->context.get()->driver_alloc(), &pool.handle);
    ...
    for (auto &state : images) {
      ...
      auto postAcquire = vulkan::alloc_command_buffer(
          surfaceImpl->context.get(), {.pool = pool});

      auto prePresent = vulkan::alloc_command_buffer(surfaceImpl->context.get(),
                                                     {.pool = pool});
