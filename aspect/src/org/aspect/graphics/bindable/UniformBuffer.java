...
public class UniformBuffer extends Bindable {
    ...
    public void uniform1f(String uniformName, float value){
        Integer offset = uniformOffsets.get(uniformName);
        System.out.println(uniformName + " " + offset);
        if(offset == null)
            throw new AspectGraphicsException("can't uniform " + uniformName + " in this uniform block");
        float[] float_array = new float[]{value};
        glBindBuffer(GL_UNIFORM_BUFFER,ID);
        glBufferSubData(GL_UNIFORM_BUFFER,offset,float_array);
        glBindBuffer(GL_UNIFORM_BUFFER,0);
    }
    ...
    public void uniformVec4(String uniformName, Vector4f vec4){
        ...
        if (offset == null)
            throw new AspectGraphicsException("can't uniform " + uniformName + " in this uniform block");
        float[] vec4_array = new float[]{vec4.x, vec4.y, vec4.z, vec4.w};
        glBindBuffer(GL_UNIFORM_BUFFER, ID);
        glBufferSubData(GL_UNIFORM_BUFFER, offset, vec4_array);
        glBindBuffer(GL_UNIFORM_BUFFER, 0);
    }
