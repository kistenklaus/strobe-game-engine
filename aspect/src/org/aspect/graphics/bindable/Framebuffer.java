...
public class Framebuffer extends Bindable {
    ...
    public Texture attachDepthTexture() {
        ...
        glBindFramebuffer(GL_FRAMEBUFFER, ID);
        int format = depthMap.isMultisampled() ? GL_TEXTURE_2D_MULTISAMPLE : GL_TEXTURE_2D;
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, format, depthMap.getID(), 0);
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
        return depthMap;
    }
