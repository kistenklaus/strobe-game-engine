...
public class RenderGraph {
    ...
    private GraphProfiler profiler = null;
    ...
    public void addPass(Pass pass) {
        pass.setupSinksAndSources();
        String passName = pass.getName();
        passes.put(passName, pass);
    }

    public void addAllPasses(Pass...passes) {
        for(Pass pass : passes)addPass(pass);
    }
    ...
    public void link() {
        if (passes.isEmpty()) throw new AspectGraphLinkingException("can't link an empty graph.");
        ...
        while (!unprocessed_links.isEmpty()) {
            ...
            for (int i = 0; i < unprocessed_links.size(); i++) {
                ...
                if (sink == null)
                    throw new AspectGraphLinkingException("error at link : " + unprocessed_links.get(i));
    ...
    public void constructPassQueue(ArrayList<Pass> queue, Pass activePass,
        ...
        if(addedPasses.contains(activePass))return;
    ...
    public void render() {
        ...
        for (Pass pass : passQueue) {
            pass.execute();
            if(profiler!=null)profiler.completed(pass.getName());
        }
    ...
    public void setProfiler(GraphProfiler profiler){
