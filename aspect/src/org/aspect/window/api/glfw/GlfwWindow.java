package org.aspect.window.api.glfw;

import org.aspect.exceptions.AspectWindowException;
import org.aspect.window.*;
import org.aspect.window.event.*;
...
public class GlfwWindow extends Window {
    ...
    public void connectWindowMouseListener(WindowMouseListener mouseListener) {
        ...
        GLFWCursorPosCallback posCallback = new GLFWCursorPosCallback() {
            @Override
            public void invoke(long window, double xpos, double ypos) {
                mouseListener.mouseMoved(
                        new WindowMousePositionEvent((float)xpos, (float)ypos));
            }
        };
        glfwSetCursorPosCallback(windowPointer, posCallback);
        ...
        GLFWMouseButtonCallback buttonCallback = new GLFWMouseButtonCallback() {
            ...
            public void invoke(long window, int button, int action, int mods) {
                ...
                if(action == GLFW_PRESS){
                    mouseListener.mouseDown(new WindowMouseButtonEvent(button, mods));
                }else if(action == GLFW_RELEASE){
                    mouseListener.mouseUp(new WindowMouseButtonEvent(button, mods));
                }
            }
        };
