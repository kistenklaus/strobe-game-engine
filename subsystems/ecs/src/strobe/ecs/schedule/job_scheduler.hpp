...
#include "strobe/ecs/schedule/job_id.hpp"
#include "strobe/ecs/schedule/job_scoreboard.hpp"
...
class JobScheduler {
public:
  using allocator = strobe::ecs::job_allocator;
  using allocator_traits = AllocatorTraits<allocator>;

  using allocator_ref = strobe::ecs::job_allocator_ref;
...
private:
  ...
  private:
    ...
    alignas(std::hardware_destructive_interference_size)
        std::atomic<uint32_t> m_invocationCounter = 0;
  ...
  using WorkerPool = WorkerPool<JobFn>;
  using semaphore = std::counting_semaphore<>;

  static_assert(SLOT_COUNT > 1);

  struct JobSlot {
    JobSlot(allocator_ref alloc, JobScheduler *scheduler)
        : fn(), scoreboard(alloc), generation(0),
          scheduler(scheduler) {}
...
public:
  JobScheduler(const strobe::ecs::allocator_ref &alloc,
               uint32_t workerCount) noexcept
      : m_alloc(std::in_place, alloc), m_workerPool(&m_alloc, workerCount) {
...
private:
  ...
  alignas(std::hardware_destructive_interference_size)
      std::atomic<JobSlot *> m_freeSlots = nullptr;
